...
class MakeFile:
    def __init__(self, text, config, template_tags, path):
        self.template_tags = template_tags
        self.text = text
        self.config = config
        self.conote_dir = self.config["FILE"]["conote_dir"]
        self.conote_file = "/".join(
            [self.conote_dir, self.config["FILE"]["conote_file"]]
        )
        self.date_time = datetime.datetime.now()
        self.template_file = path / self.config["FILE"]["template_file"]
        self.template_event = path / self.config["FILE"]["template_event"]
